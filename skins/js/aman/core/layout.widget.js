 ...
 * @version 	$Id: layout.widget.js 5352 2010-09-09 03:21:56Z mehrab $
 * @since		1.0.0
 */

'Aman.Core.Layout.Widget'.namespace();
 ...
 * @param Aman.Core.Layout.Container container Widget container
 * @constructor
 */
Aman.Core.Layout.Widget = function(id, container) {
	/**
	 * Id of widget
	 */
	this._id = id;
	 ...
	 * Widget's class name
	 */
	this._class = 'Aman.Core.Layout.Widget';
 ...
 */
Aman.Core.Layout.Widget.CACHE_LIFETIME_PARAM = '___cacheLifetime';
Aman.Core.Layout.Widget.LOAD_AJAX_PARAM 	   = '___loadAjax';
Aman.Core.Layout.Widget.PREVIEW_MODE_PARAM   = '___widgetPreviewMode';

/**
 * Getters/Setters
 */
Aman.Core.Layout.Widget.prototype.getId = function() { return this._id; };
Aman.Core.Layout.Widget.prototype.setModule = function(module) { this._module = module; };
Aman.Core.Layout.Widget.prototype.setName = function(name) { this._name = name; };
Aman.Core.Layout.Widget.prototype.setTitle = function(title) { this._title = title; };
Aman.Core.Layout.Widget.prototype.getContainer = function() { return this._container; };
Aman.Core.Layout.Widget.prototype.setContainer = function(container) { this._container = container; };
 ...
 * @return void
 */
Aman.Core.Layout.Widget.prototype.render = function(params, showOnlyContent) {
	var div = $('<div/>');
	...
	$.ajaxq('core_layout', {
		...
		success: function(response) {
			...
			$('<div class="t_a_ui_layout_editor_widget_head"><h3>' + self._title + '</h3></div>'
				...
				+ 			Aman.Core.Layout.Lang.getLang('WIDGET_CACHE') + ':<br /><input type="text" style="width: 100px" name="' + Aman.Core.Layout.Widget.CACHE_LIFETIME_PARAM + '" class="t_widget_input" />'
				+ 			'<hr /><input type="checkbox" name="' + Aman.Core.Layout.Widget.LOAD_AJAX_PARAM + '" class="t_widget_input" /> ' + Aman.Core.Layout.Lang.getLang('WIDGET_LOAD_AJAX')
				+ 		'</div>'
				+ 		'<div class="t_a_ui_layout_editor_widget_preview" id ="' + $(div).attr('id') + '_preview" style="display: none"></div>'
				+ 	'</div>'
				+	'<div class="t_a_ui_layout_editor_widget_bottom"><a href="javascript: void(0);">' + Aman.Core.Layout.Lang.getLang('WIDGET_PREVIEW') + '</a></div>'
				).appendTo($(div));
			...
			$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_widget_remove">CLOSE</a>').mousedown(function(e) {
            	...
            	if (confirm(Aman.Core.Layout.Lang.getLang('WIDGET_REMOVE_CONFIRM'))) {
			...
			$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_widget_clone">CLONE</a>').mousedown(function(e) {
            	 ...
            	 */
            	var widget = new Aman.Core.Layout.Widget(widgetId, self._container);
 ...
 * @return void
 */
Aman.Core.Layout.Widget.prototype.preview = function(showOnlyContent) {
	this._mode = 'PREVIEW';	
	var params = {};
	 ...
	 */
	params[Aman.Core.Layout.Widget.PREVIEW_MODE_PARAM] = true;
 ...
 */
Aman.Core.Layout.Widget.prototype.save = function() {
	var out = {
		cls: this._class,
		module: this._module,
		name: this._name,
	...
	$('#' + self._id).find('.t_widget_input').each(function() {
		...
		if (name == Aman.Core.Layout.Widget.LOAD_AJAX_PARAM) {
 ...
 * @return void
 */
Aman.Core.Layout.Widget.prototype.toggleMode = function(mode, showOnlyContent) {
	switch (mode) {
		case 'PREVIEW':
			if (this._mode == 'PREVIEW') {
				...
				$('#' + this._id).find('div.t_a_ui_layout_editor_widget_bottom a').html(Aman.Core.Layout.Lang.getLang('WIDGET_BACK'));
			}
			break;
		case 'CONFIG':
			...
			$('#' + this._id).find('div.t_a_ui_layout_editor_widget_bottom a').html(Aman.Core.Layout.Lang.getLang('WIDGET_PREVIEW'));
