 ...
 * @version 	$Id: layout.container.js 5082 2010-08-29 12:07:45Z mehrab $
 * @since		1.0.0
 */

'Aman.Core.Layout.Container'.namespace();
 ...
 * @constructor
 */
Aman.Core.Layout.Container = function(id, parent) {
	/** 
	 * Id of DIV container
	 */
	this._id = id;
 ...
 */
Aman.Core.Layout.Container.TOTAL_COLUMNS = 12;

/**
 * Initial height for each created container 
 */
Aman.Core.Layout.Container.ROW_HEIGHT = 150;

/**
 * This static variable store the current dragged widget
 * TODO: Use jQuery data variable
 */
Aman.Core.Layout.Container.currentDraggedWidget = null;

/**
 * Getters/setters
 */
Aman.Core.Layout.Container.prototype.getId = function() { return this._id; };
Aman.Core.Layout.Container.prototype.setParent = function(parent) { this._parent = parent; };
Aman.Core.Layout.Container.prototype.getNumColumns = function() { return this._numColumns; };
Aman.Core.Layout.Container.prototype.setNumColumns = function(numColumns) { this._numColumns = numColumns; };
Aman.Core.Layout.Container.prototype.getPosition = function() { return this._position; };
Aman.Core.Layout.Container.prototype.setPosition = function(position) { this._position = position; };
Aman.Core.Layout.Container.prototype.setBgColor = function(color) { this._bgColor = color; };
Aman.Core.Layout.Container.prototype.getBaseUrl = function() { return this._baseUrl; };
Aman.Core.Layout.Container.prototype.setBaseUrl = function(url) { this._baseUrl = url; };
 ...
 */
Aman.Core.Layout.Container.prototype.isRoot = function() {
 ...
 * @return void 
 */
Aman.Core.Layout.Container.prototype._droppable = function() {
	var self = this;
	$('#' + self._id).droppable({
		greedy: true,
		...
		drop: function(event, ui) {
			...
			if ($(item).hasClass('t_column_draggable')) {
				...
				if ((self.isRoot() && numColumns == Aman.Core.Layout.Container.TOTAL_COLUMNS) 
					|| (!self.isRoot() && numColumns < Aman.Core.Layout.Container.TOTAL_COLUMNS)) {
					
					if (numColumns != Aman.Core.Layout.Container.TOTAL_COLUMNS && self.getChildTotalColumns() > Aman.Core.Layout.Container.TOTAL_COLUMNS) {
						/**
			 ...
			 */
				...
				self._tabUuid++;
				var tab = new Aman.Core.Layout.Tabs(tabId, self);
				tab.render();
			 ...
			 */
				...
				var arr = $(item).attr('id').split('_');
				var widget = new Aman.Core.Layout.Widget(widgetId, self);
			 ...
			 */
				...
				var widgetId = self.generateWidgetId();
				var output = new Aman.Core.Layout.DefaultOutput(widgetId, self);
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.sortable = function() {
	var self = this;
	$('.t_a_ui_layout_editor_container').sortable({
        ...
        start: function(e, ui) {
			...
			Aman.Core.Layout.Container.currentDraggedWidget = self.getWidget(widgetId);
        ...
        receive: function(e, ui) {
        	 ...
        	 */
        	if (Aman.Core.Layout.Container.currentDraggedWidget != null) {
        		Aman.Core.Layout.Container.currentDraggedWidget.setContainer(self);
	        	self.addWidget(Aman.Core.Layout.Container.currentDraggedWidget);
        	}
        },
        stop: function(e, ui) {
        	/**
        	 ...
        	 */
        	if (Aman.Core.Layout.Container.currentDraggedWidget != null
        			&& Aman.Core.Layout.Container.currentDraggedWidget.getContainer().getId() != self._id) {
        		self.removeWidget(Aman.Core.Layout.Container.currentDraggedWidget);
        	}
	 ...
	 */
	if (this._numColumns == Aman.Core.Layout.Container.TOTAL_COLUMNS) {
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.resizable = function() {
	if (this._numColumns == Aman.Core.Layout.Container.TOTAL_COLUMNS) {
		return;
	}
	var self = this;
	$('#' + this._id).resizable({
		...
		maxWidth: 940,
		handles: 'e',
		//maxHeight: Aman.Core.Layout.Container.ROW_HEIGHT,
		ghost: false,
		grid: [80, 0],
		containment: 'parent',
		start: function(e, ui) {
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.updatePositionType = function() {
	var total = 0;
	...
	for (var i = 0; i < child.length; i++) {
		...
		$('#' + $(child[i]).attr('id')).find('.t_a_ui_layout_editor_container_head:first h3').html(sprintf(Aman.Core.Layout.Lang.getLang('CONTAINER_COLS'), childColumns));
 ...
 * @return int
 */
Aman.Core.Layout.Container.prototype.getChildTotalColumns = function() {
 ...
 * @return Aman.Core.Layout.Container The child container has just been added
 */
Aman.Core.Layout.Container.prototype.append = function(numColumns) {
	/**
	...
	if (numColumns != Aman.Core.Layout.Container.TOTAL_COLUMNS
			&& totalColumns > Aman.Core.Layout.Container.TOTAL_COLUMNS) {
		return null;
	}
...
//	});
	 ...
	 */
	$('#' + this._id).append(div);
	var childContainer = new Aman.Core.Layout.Container(id, this);
	 ...
	 */
	var self = this;
	$('<div class="t_a_ui_layout_editor_container_head"><h3>' + sprintf(Aman.Core.Layout.Lang.getLang('CONTAINER_COLS'), numColumns) + '</h3></div>').css('cursor', 'move').appendTo($(div));
	$('<a href="javascript: void(0)" class="t_a_ui_layout_editor_container_remove">REMOVE</a>').mousedown(function(e) {
        e.stopPropagation();
    }).click(function() {
    	if (confirm(Aman.Core.Layout.Lang.getLang('CONTAINER_REMOVE_CONFIRM'))) {
    		$(div).remove();
	...
	var minHeight = Aman.Core.Layout.Container.ROW_HEIGHT + 'px';
	...
	if (/*totalColumns == numColumns || */ numColumns == this._numColumns || this._childCount == 0) {
		...
		if (numColumns == Aman.Core.Layout.Container.TOTAL_COLUMNS) {
			minHeight += Aman.Core.Layout.Container.ROW_HEIGHT + 10;
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.addChildContainer = function(container) {
 ...
 * 
 * @param string id
 * @return Aman.Core.Layout.Container
 */
Aman.Core.Layout.Container.prototype.getChildContainer = function(id) {
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.removeChildContainer = function(container) {
 ...
 * Add widget
 * 
 * @param Aman.Core.Layout.Widget widget
 * @return void
 */
Aman.Core.Layout.Container.prototype.addWidget = function(widget) {
 ...
 * @return Aman.Core.Layout.Widget
 */
Aman.Core.Layout.Container.prototype.getWidget = function(id) {
 ...
 * 
 * @param Aman.Core.Layout.Widget widget The widget will be removed
 * @return void
 */
Aman.Core.Layout.Container.prototype.removeWidget = function(widget) {
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.incHeight = function(added) {
 ...
 */
Aman.Core.Layout.Container.prototype.generateWidgetId = function() {
	this._widgetUuid++;
 ...
 * @return string
 */
Aman.Core.Layout.Container.prototype.generateTabId = function() {
 ...
 */
Aman.Core.Layout.Container.prototype.save = function() {
	var out = {
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.load = function(data, showOnlyContent) {
	...
	for (i in data.widgets) {
		...
		switch (data.widgets[i].cls) {
			case 'Aman.Core.Layout.DefaultOutput':
				widget = new Aman.Core.Layout.DefaultOutput(widgetId, this);	
				break;
			case 'Aman.Core.Layout.Widget':
			default:
				widget = new Aman.Core.Layout.Widget(widgetId, this);
				break;
		}
 ...
 * @return void
 */
Aman.Core.Layout.Container.prototype.toggleMode = function(mode, showOnlyContent) {
	switch (mode) {
		case 'PREVIEW':
			if (showOnlyContent) {
				$('#' + this._id).children('.t_a_ui_layout_editor_container_head').hide();
