 ...
 * @version 	$Id: note.js 5082 2010-08-29 12:07:45Z mehrab $
 * @since		2.0.4
 */

'Aman.Multimedia.Note'.namespace();
'Aman.Multimedia.Note.Container'.namespace();
 ...
 */
Aman.Multimedia.Note = function(id, top, left, width, height, content, creatable, editable, removable) {
	this._top 		= top;
	this._left 		= left;
	this._width 	= width;
	this._height 	= height;
...
Aman.Multimedia.Note.prototype = {
	/**
	 * Getters/Setters
	 */
 ...
 */
Aman.Multimedia.Note.Container = function(image, photoId, options, creatable, editable, removable) {
	/** Image selector */
	this._image 	= image;
	...
	if (creatable) {
		...
		$(image).imgAreaSelect({
	        ...
	        onSelectEnd: function(img, selection) {
				var note = new Aman.Multimedia.Note(null, selection.y1, selection.x1, selection.width, selection.height, options.defaultContent, true, editable, removable);
				self.add(note);
				note.show();
			}
	    });
...
Aman.Multimedia.Note.Container.prototype = {
	/**
	 * Getters
	 */
	 ...
	 * @param Aman.Multimedia.Note note
	 */
	add: function(note) {
