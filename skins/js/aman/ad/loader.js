...
if(typeof AMAN === 'object'){
	AMAN.namespace('Aman.Ad.Loader');

	Aman.Ad.Loader = function(zoneId, containerId, route, url) {
		this._zoneId 	  = zoneId;
	...
	Aman.Ad.Loader.prototype = {
		...
		load: function() {
			 ...
			 */
			var zone = new Aman.Ad.Zone(gZone.id, gZone.name, gZone.width, gZone.height);
			...
			for (var i = 0; i < G_AD_BANNERS.length; i++) {
				...
				var banner = new Aman.Ad.Banner(gBanner.id, new Aman.Ad.Zone(gBanner.zone.id), gBanner.options, gBanner.baseUrl);
