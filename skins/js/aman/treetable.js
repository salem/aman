 ...
 * @version 	$Id: treetable.js 5082 2010-08-29 12:07:45Z mehrab $
 * @since		2.0.7
 */

'Aman.TreeTable'.namespace();

/**
 * @constructor
 */
Aman.TreeTable = function(id, rowPrefix) {
	/**
...
Aman.TreeTable.CHILD_PADDING = 40;

Aman.TreeTable.prototype = {
	/**
	 * Initialize the rows
	 * 
	 * @return void
		...
		$(row).droppable({
			...
			drop: function(event, ui) {
				...
				for (var i = movingRows.length - 1; i >= 0; i--) {
					...
					$(tr).find('td:first')
						.css('padding-left', targetPaddingLeft + delta + Aman.TreeTable.CHILD_PADDING + 'px');
	...
	getTreeData: function() {
		...
		for (var i = 0; i < numRows; i++) {
			...
			depth = parseInt($(rows[i]).find('td:first').css('padding-left')) / Aman.TreeTable.CHILD_PADDING;
	...
	_decreaseDepth: function(row, step) {
		...
		$(td).css('padding-left', paddingLeft + step * Aman.TreeTable.CHILD_PADDING + 'px');
