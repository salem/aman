 ...
 * @version 	$Id: Layout.php 5193 2010-08-30 15:01:09Z mehrab $
 * @since		1.0.0
 */
...
class Aman_Layout 
{
	 ...
	 * @var string
	 */
	const LAYOUT_DOCTYPE = '<!DOCTYPE layout SYSTEM "http://schemas.amancms.com/dtd/layout.dtd">';
	
	const WIDGET_JS_CLASS				 = 'Aman.Core.Layout.Widget';
	const WIDGET_DEFAULT_OUTPUT_JS_CLASS = 'Aman.Core.Layout.DefaultOutput';
...
//		}
	...
	private static function _saveWidget($widget) 
	{
		...
		if (count($widget['params']) > 0 
			&& isset($widget['params'][Aman_Widget::PARAM_LOAD_AJAX]) 
			&& $widget['params'][Aman_Widget::PARAM_LOAD_AJAX]['value'] != '') 
		{
			$load = 'ajax';
		}
		unset($widget['params'][Aman_Widget::PARAM_LOAD_AJAX]);
		 ...
		 */
		$lifetime = null;
		if (isset($widget['params'][Aman_Widget::PARAM_CACHE_LIFETIME])) {
			$lifetime = $widget['params'][Aman_Widget::PARAM_CACHE_LIFETIME]['value'];
			$lifetime = ltrim($lifetime, ' ');
			$lifetime = rtrim($lifetime, ' ');
			
			unset($widget['params'][Aman_Widget::PARAM_CACHE_LIFETIME]);
		}
	...
	private static function _loadWidget($widgetNode) 
	{
		...
		if (isset($attrs['load']) && ((string)$attrs['load'] == 'ajax')) {
			$return['params'][Aman_Widget::PARAM_LOAD_AJAX] = array('value' => true);
		}
		...
		if ($widgetNode->cache) {
			...
			$return['params'][Aman_Widget::PARAM_CACHE_LIFETIME] = array('value' => (string)$attrs['lifetime']);
		}
