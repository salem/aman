 ...
 * @version 	$Id: Translator.php 5304 2010-09-03 10:39:14Z mehrab $
 * @since		2.0.3
 */

class Aman_View_Helper_Translator extends Zend_View_Helper_Abstract
{
	...
	public function translator($key = null, $module = null)
	{
		 ...
		 * $file = AMAN_ROOT_DIR . DS . 'languages' . DS . $lang . DS . 'application' . DS . 'modules' . DS . $module . DS . 'languages' . DS . 'lang.' . $lang . '.ini';		 
		 */
	...
	public function widget($key)
	{
		 ...
		 * __call() of Aman_Widget
		 */
		if (!Zend_Registry::isRegistered(Aman_Widget::CURRENT_WIDGET_KEY)) {
			return $key;
		}
		$widget = Zend_Registry::get(Aman_Widget::CURRENT_WIDGET_KEY);
		if (null == $widget || !($widget instanceof Aman_Widget)) {
			return $key;
		}
		...
		/**
		 * If we want to use the file in languages directory
		 * $file = AMAN_ROOT_DIR . DS . 'languages' . DS . $lang . DS . 'application' . DS . 'modules' . DS . strtolower($widget->getModule()) . DS . 'widgets' . DS . strtolower($widget->getName()) . DS . 'lang.' . $lang . '.ini';
		 */
		
	 	$file = AMAN_APP_DIR . DS . 'modules' . DS . strtolower($widget->getModule()) . DS  . 'widgets' . DS . strtolower($widget->getName()) . DS . 'lang.' . $lang . '.ini';
		if (file_exists($file) && file_get_contents($file) != '') {
