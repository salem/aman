 ...
 * @version 	$Id: LayoutLoader.php 5376 2010-09-10 07:40:05Z mehrab $
 * @since		1.0.0
 */

class Aman_View_Helper_LayoutLoader extends Zend_View_Helper_Abstract 
{
	...
	public function layoutLoader($cache = false)
	{
		...
		if ($cache) {
			$cacheFile = AMAN_TEMP_DIR . DS . 'cache' . DS . 'layout_' . $routeName . '.php';
				...
				case XMLReader::END_ELEMENT . '_widget':
					if ($module != null) {
						if ('php' == $load) {
							...
							$this->_inlineScripts[] = "Aman.Core.Widget.Loader.queue('$module', '$widgetName', '$data', '$id');"; 
						}
					}
					/**
					 * Reset variables
