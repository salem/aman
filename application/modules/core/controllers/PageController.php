 ...
 * @version 	$Id: PageController.php 5376 2010-09-10 07:40:05Z mehrab $
 * @since		1.0.0
 */
...
class Core_PageController extends Zend_Controller_Action 
{
	...
	public function layoutAction() 
	{
		 ...
		 */
		$jsonData = null;
		$jsonFile = AMAN_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $pageName . '.json';
		$xmlFile  = AMAN_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $pageName . '.xml';
		if (file_exists($jsonFile)) {
			 ...
			 */
			$array = Aman_Layout::load($xmlFile);
	...
	public function savelayoutAction() 
	{
		...
		if ($request->isPost()) {
			 ...
			 */
			$jsonFile = AMAN_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $page . '.json';
			$file 	  = fopen($jsonFile, 'w');
			fwrite($file, $jsonLayout);
			fclose($file);
			
			$xmlFile = AMAN_APP_DIR . DS . 'templates' . DS . $template . DS . 'layouts' . DS . $page . '.xml';
			Aman_Layout::save($xmlFile, $layout);
	...
	public function widgetsAction() 
	{
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Aman_Utility_PaginatorAdapter($widgets, $numWidgets));
