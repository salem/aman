 ...
 * @version 	$Id: HookLoader.php 3971 2010-07-25 10:26:42Z mehrab $
 * @since		1.0.0
 */
...
class Core_Controllers_Plugin_HookLoader extends Zend_Controller_Plugin_Abstract 
{
	public function preDispatch(Zend_Controller_Request_Abstract $request)
	{
		...
		$targetDao = Aman_Model_Dao_Factory::getInstance()->setModule('core')->getTargetDao();
		...
		if ($targets) {
			foreach ($targets as $target) {
				 ...
				 */
				if (class_exists($hookClass) && (($hook = new $hookClass()) instanceof Aman_Hook)) {
					Aman_Hook_Registry::getInstance()->register($target->target_name, array($hook, $target->hook_type));
				}
			}
		}
	}
