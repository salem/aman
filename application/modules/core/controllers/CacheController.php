 ...
 * @version 	$Id: CacheController.php 4933 2010-08-25 07:21:04Z mehrab $
 * @since		1.0.0
 */
...
class Core_CacheController extends Zend_Controller_Action 
{
	...
	public function htmlAction() 
	{
		...
		$content = Aman_Cache_File::fromCache($type, $key);
	...
	public function clearAction() 
	{
		...
		if ($request->isPost()) {
			...
			switch ($type) {
				 ...
				 */
				case 'css':
					Core_Services_FileCache::clear(AMAN_TEMP_DIR . DS . 'cache' . DS . 'css');
					break;
				case 'js':
					Core_Services_FileCache::clear(AMAN_TEMP_DIR . DS . 'cache' . DS . 'js');
					break;
				case 'pages':
					Core_Services_FileCache::clear(AMAN_TEMP_DIR . DS . 'cache' . DS . 'url');
					break;
			}
	...
	public function configAction()
	{
		...
		$pageDao = Aman_Model_Dao_Factory::getInstance()->setModule('core')->getPageDao();
		...
		$appFile   = AMAN_APP_DIR . DS . 'config' . DS . 'application.ini';
		...
		if ($request->isPost()) {
			...
			if (is_array($selectedPages) && count($selectedPages) > 0) {
				 ...
				 */
				$appConfig['production']['resources']['frontController']['plugins']['urlCache'] = 'Aman_Controller_Plugin_UrlCache';
			...
			if ($appConfig['cache']['compress']['html'] == 'true') {
				$appConfig['production']['resources']['frontController']['plugins']['htmlCompressor'] = 'Aman_Controller_Plugin_HtmlCompressor';
			} else {
