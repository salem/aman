 ...
 * @version 	$Id: InstallController.php 5377 2010-09-10 07:44:37Z mehrab $
 * @since		2.0.1
 */
...
class Core_InstallController extends Zend_Controller_Action 
{
	...
	private static $_DATABASES = array(
		...
		'mysql'	=> array(
			...
			'data'		 => '/install/amancms_sample_db_mysql.sql',
		), 
		/**
	...
	public function indexAction()
	{
		...
		$currentLang = Aman_Config::getConfig()->web->language->code;
		...
		if ($request->isPost()) {
			...
			if ('upload' == $act) {
				...
				$result = Aman_Language::upload($_FILES['Filedata']);
				...
				if ($currentLang != $lang) {
					$file 	= AMAN_APP_DIR . DS . 'config' . DS . 'application.ini';			
		...
		$this->view->assign('languages', Aman_Language::getAvailableLanguages());
	...
	public function requirementAction() 
	{
		...
		$writableFiles = array();
		foreach ($files as $f) {
			$writableFiles[$f] = is_writeable(AMAN_ROOT_DIR . DS . $f);
	...
	public function configAction() 
	{
		 ...
		 */
		$subDirs   = Aman_Utility_File::getSubDir(AMAN_APP_DIR . DS . 'templates');
		$templates = array();
		foreach ($subDirs as $dir) {
			/**
			 * Load template info
			 */
			$file = AMAN_APP_DIR . DS . 'templates' . DS . $dir . DS . 'about.xml';
			if (!file_exists($file)) {
				continue;
			}
		...
		if ('saveConfig' == $mode || $request->isPost()) {
			...
			if ($request->getPost('cacheType') != '') {
				...
				switch (strtolower($request->getPost('cacheType'))) {
					case 'file':
						$config['cache']['backend']['options']['cache_dir'] = '{AMAN_TEMP_DIR}{DS}cache';
						break;
					case 'memcache':
			...
			if (get_magic_quotes_gpc()) {
				$config['production']['resources']['frontController']['plugins']['magicQuote'] = 'Aman_Controller_Plugin_MagicQuote';	
			} else {
			...
			if ($mode != 'install') {
				...
				try {
					$moduleDirs = Aman_Utility_File::getSubDir(AMAN_APP_DIR . DS . 'modules');
					...
					$templateDao  = Aman_Model_Dao_Factory::getInstance()->setModule('core')->getTemplateDao();
					 ...
					 * Finally, init data
					 */
					$dbFile = AMAN_ROOT_DIR . DS . 'install' . DS . 'db.xml';
					...
					if (file_exists($dbFile)) {
						...
						if (is_array($xpath) && count($xpath) > 0) {
							foreach ($xpath as $query) {
								...
								Aman_Db_Connection::factory()->query($q);
							}
						}
					}
					...
					if ($importSampleData) {
						...
						if ($importer != null && $file != null) {
							$importer->import(AMAN_ROOT_DIR . $file);
						}
					}
				} catch (Exception $ex) {
					$ok = false;
	...
	public function completeAction()
	{
		...
		if ($request->isPost()) {
			 ...
			 * @since 2.0.3
			 */
			$config['install']['version'] = Aman_Version::getVersion();
			...
			$conn = Aman_Db_Connection::factory()->getMasterConnection();
			$userDao = Aman_Model_Dao_Factory::getInstance()->setModule('core')->getUserDao();
