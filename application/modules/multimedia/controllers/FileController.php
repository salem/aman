 ...
 * @version 	$Id: FileController.php 5498 2010-09-22 01:44:46Z mehrab $
 * @since		1.0.0
 */
...
class Multimedia_FileController extends Zend_Controller_Action 
{
	...
	public function init() 
	{
		 ...
		 * @since 2.0.2
		 */
		Aman_Hook_Registry::getInstance()->register('Multimedia_File_Add_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(), 
			'show',
			array('file_id', 'multimedia_file_details', 'multimedia_tag_file')
		));
		Aman_Hook_Registry::getInstance()->register('Multimedia_File_Edit_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(),
			'show',
			array('file_id', 'multimedia_file_details', 'multimedia_tag_file')
		));
	...
	public function detailsAction() 
	{
		...
		$noteDao = Aman_Model_Dao_Factory::getInstance()->setModule('multimedia')->getNoteDao();
	...
	public function addAction() 
	{
		...
		if ($request->isPost()) {
			...
			$file = new Multimedia_Models_File(array(
				'title'	 			=> $title,
				'slug' 				=> Aman_Utility_String::removeSign($title, '-', true),
			...
			if ($fileId > 0) {
				 ...
				 * @since 2.0.2
				 */
				Aman_Hook_Registry::getInstance()->executeAction('Multimedia_File_Add_Success', $fileId);
		...
		if ($request->isPost()) {
			...
			if ($keySection && $newValue) {
				 ...
				 * Execute hooks
			 	 * @since 2.0.2
			 	 */
				Aman_Hook_Registry::getInstance()->executeAction('Multimedia_File_Edit_Success', $fileId);
	...
	public function editorAction()
	{
		...
		if ($request->isPost() && $load == 'edit') {
			...
			$ret 	= $des;
			$des 	= AMAN_ROOT_DIR . DS .str_replace($baseUrl, '', $des);
			$source = AMAN_ROOT_DIR . DS .str_replace($baseUrl, '', $source);
			
			/**
			 * Get config
			 */
			$config  = Aman_Module_Config::getConfig('upload');
			$tool 	 = $config->thumbnail->tool;
			$service = null;
			switch (strtolower($tool)) {
				case 'imagemagick':
					$service = new Aman_Image_ImageMagick();
					break;
				case 'gd':
					$service = new Aman_Image_GD();
					break;
			}
	...
	public function listAction() 
	{
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Aman_Utility_PaginatorAdapter($files, $numFiles));
