 ...
 * @version 	$Id: SetController.php 5099 2010-08-29 20:09:17Z mehrab $
 * @since		1.0.0
 */
...
class Multimedia_SetController extends Zend_Controller_Action 
{
	...
	public function init() 
	{
		 ...
		 * @since 2.0.2
		 */
		Aman_Hook_Registry::getInstance()->register('Multimedia_Set_Add_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(), 
			'show',
			array('set_id', 'multimedia_set_details', 'multimedia_tag_set')
		));
		Aman_Hook_Registry::getInstance()->register('Multimedia_Set_Edit_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(),
			'show',
			array('set_id', 'multimedia_set_details', 'multimedia_tag_set')
		));
	...
	public function addAction() 
	{
		...
		if ($request->isPost()) {
			...
			if ($setId > 0) {
				 ...
				 * @since 2.0.2
				 */
				Aman_Hook_Registry::getInstance()->executeAction('Multimedia_Set_Add_Success', $setId);
		...
		if ($request->isPost()) {
			...
			if ($keySection && $newValue) {
				...
				if ($setId > 0) {
					 ...
					 * @since 2.0.2
					 */
					Aman_Hook_Registry::getInstance()->executeAction('Multimedia_Set_Edit_Success', $setId);
	...
	public function listAction() 
	{
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Aman_Utility_PaginatorAdapter($sets, $numSets));
		$paginator->setCurrentPageNumber($pageIndex);
		$paginator->setItemCountPerPage($perPage);
