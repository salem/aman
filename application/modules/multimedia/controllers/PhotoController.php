 ...
 * @version 	$Id: PhotoController.php 4976 2010-08-26 03:28:23Z mehrab $
 * @since		1.0.0
 */
	...
	public function uploadAction() 
	{
		...
		if ($request->isPost()) {
			...
			if ($watermark) {
				...
				$file 	= AMAN_APP_DIR . DS . 'modules' . DS . 'upload' . DS . 'config' . DS . 'config.ini';		
			...
			if ($title && $title != ',') {
				...
				for ($i = 1; $i < count($arrTitle); $i++) {
					...
					$file = new Multimedia_Models_File(array(
						'title' 			=> $imageName,
						'slug' 				=> Aman_Utility_String::removeSign($imageName, '-', true),
