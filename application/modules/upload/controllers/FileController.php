...
class Upload_FileController extends Zend_Controller_Action 
{
	...
	public function browseAction()
	{
		...
		$fullPath = AMAN_ROOT_DIR . DS . $path;
		if (!file_exists($fullPath)) {
			$fullPath = AMAN_ROOT_DIR . DS . 'upload';
			$path     = 'upload';
		}
	...
	public function uploadAction()
	{
		 ...
		 * Prepare folders
		 */
		$dir  = AMAN_ROOT_DIR . DS . 'upload';
		$path = implode(DS, array($module, $userName, date('Y'), date('m')));
		Aman_Utility_File::createDirs($dir, $path);
		...
		if ($thumbnailSizes != 'none') {
			...
			foreach ($thumbnailSizes as $s) {
				 ...
				 */
				if ($watermark) {
					$service->setWatermarkFont(AMAN_ROOT_DIR . '/data/upload/' . self::WATERMARK_FONT);
