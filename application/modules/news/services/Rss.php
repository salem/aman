 ...
 * @version 	$Id: Rss.php 5048 2010-08-28 18:15:15Z mehrab $
 */
...
class News_Services_Rss 
{
	...
	public static function feed($categoryId = null, $lang) 
	{
		...
		$prefix  = AMAN_TEMP_DIR . DS . 'cache' . DS . $lang . '_news_rss_';
		
		$file 	 = (null == $categoryId)
					? $prefix . 'latest.xml' 
					: $prefix . 'category_' . $categoryId . '.xml';

		if (Aman_Cache_File::isFileNewerThan($file, time() - self::CACHE_TIME)) {
		...
		$newsConfig = Aman_Module_Config::getConfig('news');			
		...
		$generator = ($newsConfig->rss->channel_generator != '') 
						...
						: 'AmanCMS v' . Aman_Version::getVersion(); 
