 ...
 * @version 	$Id: ArticleController.php 5462 2010-09-20 04:45:41Z mehrab $
 * @since		1.0.0
 */
...
class News_ArticleController extends Zend_Controller_Action 
{
	...
	public function init() 
	{
		 ...
		 * @since 2.0.2
		 */
		Aman_Hook_Registry::getInstance()->register('News_Article_Add_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(), 
			'show', 
			array('article_id', 'news_article_details', 'news_tag_article')
		));
		Aman_Hook_Registry::getInstance()->register('News_Article_Edit_ShowSidebar', array(
			new Tag_Hooks_Tagger_Hook(), 
			'show', 
			array('article_id', 'news_article_details', 'news_tag_article')
		));
		Aman_Hook_Registry::getInstance()->register(
	...
	public function categoryAction() 
	{
		...
		if (null == $category) {
			throw new Aman_Exception_NotFound();
		}
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Aman_Utility_PaginatorAdapter($articles, $numArticles));
	...
	public function detailsAction()
	{
		 ...
		 * Format content
		 */
		$article->content = Aman_Hook_Registry::getInstance()->executeFilter('News_Article_Details_FormatContent', $article->content);
		...
		if (!$preview && $article->status != 'draft') {
			$cookieName = '__aman_news_details_numviews';
			$viewed = false;
	...
	public function addAction() 
	{
		...
		$conn = Aman_Db_Connection::factory()->getMasterConnection();
		$articleDao  = Aman_Model_Dao_Factory::getInstance()->setModule('news')->getArticleDao();
		$revisionDao = Aman_Model_Dao_Factory::getInstance()->setModule('news')->getRevisionDao();
		$articleDao->setDbConnection($conn);
		$revisionDao->setDbConnection($conn);
		
		$descriptionPrefix = Aman_Module_Config::getConfig('news')->general->description_prefix;
		...
		if ($request->isPost()) {
			...
			if ($id > 0) {
				...
				$translationDao = Aman_Model_Dao_Factory::getInstance()->setModule('core')->getTranslationDao();
				...
				if ($preview) {
					 ...
					 * @since 2.0.2
					 */
					Aman_Hook_Registry::getInstance()->executeAction('News_Article_Add_Success', $id);
	...
	public function editAction() 
	{
		...
		if ($request->isPost()) {
			...
			if ($preview) {
				 ...
				 * @since 2.0.2
				 */
				Aman_Hook_Registry::getInstance()->executeAction('News_Article_Edit_Success', $articleId);
	...
	public function hotAction() 
	{
		 ...
		 * @since 2.0.8
		 */
		$lang = $request->getParam('lang', Aman_Config::getConfig()->web->lang);
