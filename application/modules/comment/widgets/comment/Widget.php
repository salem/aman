 ...
 * @license		GNU GPL license, see http://www.amancms.com/license.txt or license.txt
 * @version 	$Id: Widget.php 5272 2010-09-01 09:49:55Z hoangninh $
 * @since		2.0.1
 */

class Comment_Widgets_Comment_Widget extends Aman_Widget 
{
	protected function _prepareShow() 
	{
		$perPage 	  = $this->_request->getParam('limit', 10);
		$allowComment = $this->_request->getParam('allow_comment');
		$isPreviewing = $this->_request->getParam(Aman_Widget::PARAM_PREVIEW_MODE);
		 ...
		 * Paginator
		 */
		$paginator = new Zend_Paginator(new Aman_Utility_PaginatorAdapter($comments, $numComments));
		...
		if ($this->_request->isPost('tCommentEmail') && $allowComment) {
			...
			$ok = false;
			$moduleConfig = Aman_Module_Config::getConfig('comment');	
			if ($moduleConfig != null && $moduleConfig->akismet->api_key && $moduleConfig->akismet->api_key != '') {
				$akismetService = new Zend_Service_Akismet($moduleConfig->akismet->api_key, Aman_Config::getConfig()->web->url->base);
     			$params = array(
     				'user_ip' 			   => $comment->ip,
